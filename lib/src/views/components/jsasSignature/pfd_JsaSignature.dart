...
class PDFJsaSignature extends StatefulWidget {
  ...
  @override
  _PDFJsaSignatureState createState() => _PDFJsaSignatureState();
}

class _PDFJsaSignatureState extends State<PDFJsaSignature> {
  ...
  Future downloadPDF() async {
    try {
      //"?id=${widget.workerOrderID}"
      final String urlApi =
          '${api.api}/${api.pdfSingJSA}/${widget.workerOrderID}/1';
      //print(urlApi);
      final header = {
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text('JSA #${widget.workerOrderID}'),
          centerTitle: true,
          leading: IconButton(
              onPressed: () {
                //
  ...
  inputName() {
    return Container(
      ...
      child: PhysicalModel(
        ...
        child: TextFormField(
          enabled: true,
          controller: widget.parentController.nameTxt,
          style: ThemeWhite().labelHomeTitles(Colors.black),
          onChanged: (value) {
            if (widget.parentController.nameTxt.text.isEmpty) {
          ...
          autofocus: false,
          decoration: StyleInput().inputTextSyle('Name', Colors.white),
        ),
      ),
    );
  }
